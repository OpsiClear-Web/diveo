...
import { useTheme, space } from "../utils/theme";

// Shared diVeo brand lockup (logo glyph + wordmark), mirroring the gsav-hosting
// web header (28px mark + Roboto-900 "diVeo"). The neutral third blade is themed
// so the mark stays legible in both dark and light; the coral/gold/white blades
// are fixed brand colors. The source art lives at user-space origin (1717,1574),
// so the viewBox starts there (equivalent to the source's translate) and scales
// the 808-unit art down to `logoSize` — works on native and react-native-web.
export function Brand({ logoSize = 26, fontSize = 20 }: { logoSize?: number; fontSize?: number }) {
  ...
  return (
    <View style={styles.row}>
      ...
      <Text style={[styles.wordmark, { color: theme.text, fontSize }]}>diVeo</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  wordmark: { fontFamily: "Roboto_900Black", letterSpacing: -0.4 },
});
