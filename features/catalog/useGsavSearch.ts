...
import { searchCatalogScenes, type GsavContentItem } from "./catalogAdapter";

const HISTORY_KEY = "gsav_search_history";
const MAX_HISTORY = 20;

/**
 * Native GSAV catalog search: full-text search via the catalog `q` param.
...
export function useGsavSearch() {
  ...
  const search = useCallback(async (q: string) => {
    ...
    setError(null);
    try {
      const page = await searchCatalogScenes(trimmed);
      if (reqId === reqRef.current) {
        setResults(page.videos);
        setSearched(true);
      }
    } catch (e) {
      if (reqId === reqRef.current) setError(formatGsavCatalogError(e, "Search failed."));
    } finally {
